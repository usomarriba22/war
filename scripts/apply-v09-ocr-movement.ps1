...
# CON War Room v0.9 — OCR Foundation + Movement Advisor Integrated
...
New-Item -ItemType Directory -Force -Path `
  ...
  ".\02-apps\warroom-web\public\assets\resources", `
  ".\02-apps\warroom-web\_legacy" | Out-Null

# Backup
$stamp = Get-Date -Format "yyyyMMdd-HHmmss"
Copy-Item ".\02-apps\warroom-web\src\main.jsx" ".\02-apps\warroom-web\_legacy\main-$stamp.jsx" -Force -ErrorAction SilentlyContinue
Copy-Item ".\02-apps\warroom-web\src\styles.css" ".\02-apps\warroom-web\_legacy\styles-$stamp.css" -Force -ErrorAction SilentlyContinue
Copy-Item ".\02-apps\warroom-web\src\lib\data.js" ".\02-apps\warroom-web\_legacy\data-$stamp.js" -Force -ErrorAction SilentlyContinue
Copy-Item ".\02-apps\warroom-api\app\main.py" ".\02-apps\warroom-api\app\main-$stamp.py.bak" -Force -ErrorAction SilentlyContinue

# Package con OCR client-side
$packageJson = @'
{
...
'@
[System.IO.File]::WriteAllText("$PWD\02-apps\warroom-web\package.json", $packageJson, $Utf8NoBom)

# SVG pictorial icons
function WriteSvg {
  param([string]$Path, [string]$Code, [string]$Label, [string]$Shape)
  $svg = @"
<svg xmlns="http://www.w3.org/2000/svg" width="128" height="128" viewBox="0 0 128 128">
  ...
  $Shape
  <text x="64" y="106" text-anchor="middle" font-size="13" font-family="Consolas, monospace" font-weight="900" fill="#d8f3ff">$Label</text>
</svg>
"@
  [System.IO.File]::WriteAllText($Path, $svg, $Utf8NoBom)
}

WriteSvg ".\02-apps\warroom-web\public\assets\resources\supplies.svg" "SUP" "SUPPLIES" '<path d="M43 46h42v31H43z M48 41h32l5 5H43z M64 41v36" fill="none" stroke="#d8f3ff" stroke-width="5" stroke-linejoin="round" filter="url(#glow)"/>'
WriteSvg ".\02-apps\warroom-web\public\assets\resources\components.svg" "CMP" "PARTS" '<path d="M64 32v11 M64 81v11 M32 64h11 M81 64h11 M42 42l8 8 M78 78l8 8 M86 42l-8 8 M50 78l-8 8" stroke="#d8f3ff" stroke-width="5" stroke-linecap="round"/><circle cx="64" cy="64" r="18" fill="none" stroke="#d8f3ff" stroke-width="5"/>'
WriteSvg ".\02-apps\warroom-web\public\assets\resources\fuel.svg" "FUL" "FUEL" '<path d="M50 34h24v55H50z M74 44h10l6 8v31c0 5-7 5-7 0V62h-9" fill="none" stroke="#d8f3ff" stroke-width="5" stroke-linejoin="round"/><path d="M56 48h12" stroke="#3dff9b" stroke-width="5" stroke-linecap="round"/>'
WriteSvg ".\02-apps\warroom-web\public\assets\resources\electronics.svg" "ELC" "ELECTRO" '<path d="M67 30L46 66h17l-3 32 24-42H67z" fill="#d8f3ff" stroke="#3dff9b" stroke-width="3" stroke-linejoin="round" filter="url(#glow)"/>'
WriteSvg ".\02-apps\warroom-web\public\assets\resources\rares.svg" "RAR" "RARES" '<path d="M42 52l12-16h20l12 16-22 37z" fill="none" stroke="#d8f3ff" stroke-width="5" stroke-linejoin="round"/><path d="M42 52h44 M54 36l10 16 10-16 M64 52v37" stroke="#3dff9b" stroke-width="4"/>'
WriteSvg ".\02-apps\warroom-web\public\assets\resources\manpower.svg" "MAN" "TROOPS" '<path d="M42 82c4-17 40-17 44 0 M50 50c3-18 25-18 28 0 M48 53h32 M55 43h18" fill="none" stroke="#d8f3ff" stroke-width="5" stroke-linecap="round"/><circle cx="64" cy="58" r="15" fill="none" stroke="#3dff9b" stroke-width="4"/>'
WriteSvg ".\02-apps\warroom-web\public\assets\resources\money.svg" "MON" "MONEY" '<rect x="38" y="45" width="52" height="36" rx="6" fill="none" stroke="#d8f3ff" stroke-width="5"/><circle cx="64" cy="63" r="9" fill="none" stroke="#3dff9b" stroke-width="4"/><path d="M45 54h8 M75 72h8" stroke="#d8f3ff" stroke-width="4" stroke-linecap="round"/>'
...
'@
[System.IO.File]::WriteAllText("$PWD\02-apps\warroom-web\src\lib\data.js", $dataJs, $Utf8NoBom)
...
function App() {
  ...
  async function runOcrTopBar() {
    ...
    try {
      ...
      crop.width = 1600; crop.height = 190;
      ctx.fillStyle = "#000"; ctx.fillRect(0, 0, crop.width, crop.height);
      ctx.drawImage(src, 0, 0, src.width, Math.round(src.height * 0.18), 0, 0, crop.width, crop.height);
      ...
      setOcrText(text);
      setOcrNumbers(parseOcrNumbers(text));
    } catch (e) { setOcrText(`OCR error: ${e.message}`); }
    setOcrBusy(false);
  }
...
function CaptureTab({ videoRef, canvasRef, bigCanvasRef, cropCanvasRef, captureOpen, setCaptureOpen, captureStatus, captureMode, setCaptureMode, startCapture, stopCapture, addMark, marks, setMarks, markLabel, setMarkLabel, runOcrTopBar, ocrBusy, ocrText, ocrNumbers, applyOcrToResources }) {
  return <section className="capturePage"><Panel title="Capture Center + OCR" right={captureStatus}><video ref={videoRef} className="hiddenVideo"/><div className="captureStage" onPointerDown={addMark}><canvas ref={canvasRef} width="1280" height="720" className="captureCanvas"/>{marks.map((m,i)=><div className="captureMark" key={i} style={{left:`${m.x}%`,top:`${m.y}%`}}><span>{i+1}</span><em>{m.label}</em></div>)}</div><div className="row"><input className="markInput" value={markLabel} onChange={e=>setMarkLabel(e.target.value)} placeholder="texto del marcador"/><button onClick={startCapture}><Video size={16}/> Conectar pantalla</button><button onClick={()=>setCaptureOpen(true)}><Eye size={16}/> Ampliar</button><button onClick={()=>setCaptureMode(captureMode==="fit"?"fill":"fit")}>Modo {captureMode}</button><button onClick={()=>setMarks([])}>Limpiar marcas</button><button className="danger" onClick={stopCapture}>Parar</button></div><p className="hint">Click sobre la captura crea marcador tactico. Captura observa, no controla el juego.</p></Panel><Panel title="OCR Top Bar"><canvas ref={cropCanvasRef} width="1600" height="190" className="ocrCrop"/><div className="row"><button onClick={runOcrTopBar} disabled={ocrBusy}>{ocrBusy ? "OCR..." : "Leer barra superior"}</button><button onClick={applyOcrToResources}>Aplicar OCR a recursos</button></div><p className="hint">Numeros detectados: {ocrNumbers.join(" / ") || "sin datos"}</p><textarea className="ocrText" value={ocrText} onChange={e=>{}} readOnly/></Panel>{captureOpen && <div className="captureModal"><div className="captureModalTop"><strong>LIVE GAME FEED</strong><div className="row"><button onClick={()=>setCaptureMode(captureMode==="fit"?"fill":"fit")}>Modo {captureMode}</button><button className="danger" onClick={()=>setCaptureOpen(false)}>Cerrar</button></div></div><div className="captureBigStage" onPointerDown={addMark}><canvas ref={bigCanvasRef} width="1920" height="1080" className="captureBig"/>{marks.map((m,i)=><div className="captureMark big" key={i} style={{left:`${m.x}%`,top:`${m.y}%`}}><span>{i+1}</span><em>{m.label}</em></div>)}</div></div>}</section>;
}
...
'@
[System.IO.File]::WriteAllText("$PWD\02-apps\warroom-web\src\main.jsx", $mainJsx, $Utf8NoBom)
...
.frontItem,.feedItem,.moveCard{border-left:3px solid var(--cyan);background:rgba(54,217,255,.055);border-radius:12px;padding:10px;margin-bottom:8px}.frontItem p,.feedItem p,.moveCard p{margin:5px 0;color:var(--muted);font-size:12px}.frontItem span{font-size:11px;border:1px solid var(--border);border-radius:999px;padding:4px 8px;color:var(--cyan)}.advisor{min-height:310px;color:var(--green)}.movementGrid,.dataGrid,.capturePage{display:flex;flex-direction:column;gap:14px}.moveCols{display:grid;grid-template-columns:1fr 1fr;gap:14px}
.hiddenVideo{display:none}.captureStage,.captureBigStage{position:relative;width:100%;background:#000;border-radius:18px;overflow:hidden;border:1px solid rgba(54,217,255,.25);cursor:crosshair}.captureCanvas{width:100%;height:560px;background:#000;display:block}.captureBig{width:100%;height:100%;background:#000;display:block}.captureMark{position:absolute;transform:translate(-50%,-50%);display:flex;align-items:center;gap:7px;pointer-events:none;filter:drop-shadow(0 0 8px rgba(61,255,155,.65))}.captureMark span{width:28px;height:28px;border-radius:999px;background:rgba(61,255,155,.18);border:1px solid #3dff9b;color:#3dff9b;display:inline-flex;align-items:center;justify-content:center;font-weight:950}.captureMark em{font-style:normal;font-size:12px;color:#d8f3ff;background:rgba(0,0,0,.72);border:1px solid rgba(54,217,255,.35);border-radius:999px;padding:5px 8px}.captureMark.big span{width:34px;height:34px}.captureMark.big em{font-size:14px}.markInput{max-width:220px}.captureModal{position:fixed;z-index:9999;inset:0;background:rgba(0,0,0,.92);display:grid;grid-template-rows:auto 1fr;padding:20px;gap:12px}.captureModalTop{display:flex;justify-content:space-between;align-items:center;border:1px solid var(--border);background:rgba(7,18,30,.92);border-radius:16px;padding:12px}.ocrCrop{width:100%;height:110px;background:#000;border:1px solid var(--border);border-radius:12px}.ocrText{min-height:120px}
@media(max-width:1400px){.app{grid-template-columns:280px 1fr}.rightbar{grid-column:1/-1}.metrics{grid-template-columns:repeat(3,1fr)}}@media(max-width:900px){.app,.metrics,.resourceGrid,.chartGrid,.formGrid.two,.formGrid.three,.formGrid.four,.moveCols{grid-template-columns:1fr}.topbar{align-items:flex-start;flex-direction:column}}
'@
[System.IO.File]::WriteAllText("$PWD\02-apps\warroom-web\src\styles.css", $css, $Utf8NoBom)

# API v0.9 movement endpoint
$api = @'
...
Write-Host "v0.9 OCR + Movement integrado aplicado."
Write-Host "Siguiente: git add . ; git commit ; git push ; build api/web ; rollout restart."
